...
export default function Page() {
  ...
  return <main>
    ...
    {data.checks && <><section className={`result ${data.ready ? "pass" : "blocked"}`}><h2>{data.ready ? "READY / 已具备验收证据" : "NOT READY / 尚缺真实证据"}</h2><p>{data.property?.name} · {data.property?.address}</p>{data.acceptance ? <p className="certificate">✓ Certified {new Date(data.acceptance.certifiedAt).toLocaleString()} · SHA-256 {data.acceptance.evidenceSha256.slice(0, 16)}…</p> : data.ready && <button onClick={() => void certify()} disabled={busy}>Certify immutable acceptance / 生成不可变验收记录</button>}</section>

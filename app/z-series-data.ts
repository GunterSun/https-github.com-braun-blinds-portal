...
export const Z_SERIES_SOURCE = {
  workbook: "Z_Series_Customer_Price_List_CN_EN.xlsx",
...
export const Z_SERIES_CATALOG: readonly ZSeriesCatalogItem[] = Z_SERIES_FABRICS.flatMap((fabric) => {
  ...
  return Z_SERIES_CONFIGS.flatMap((config, configIndex) => {
    const retail = prices[configIndex];
    ...
    return [{
      ...
      descriptionEn,
      productCode,
      system: `${systemZh} / ${systemEn}`,
      systemZh,
      systemEn,
      style: `${styleZh} / ${styleEn}`,
      styleZh,
      styleEn,
      structure: `${structureZh} / ${structureEn}`,
      structureZh,
      structureEn,
      construction: `${constructionZh} / ${constructionEn}`,
      constructionZh,
      constructionEn,
      minWidth,
      maxWidth,
...
if (Z_SERIES_CATALOG.length !== Z_SERIES_SOURCE.expectedRows) {
  throw new Error(`Z-Series catalog integrity error: expected ${Z_SERIES_SOURCE.expectedRows}, got ${Z_SERIES_CATALOG.length}`);
}

export function findZSeriesItem(fabricCode: string, productCode: string) {
  const fabric = fabricCode.trim().toUpperCase();
  const product = productCode.trim().toUpperCase();
  return Z_SERIES_CATALOG.find((item) => item.fabricCode === fabric && item.productCode === product) ?? null;
}

export function zSeriesWholesalePrice(retail: number, discountPercent: number) {
  if (!Number.isFinite(retail) || retail < 0) throw new Error("Invalid Z-Series retail price");
  if (!Number.isFinite(discountPercent) || discountPercent < 0 || discountPercent > 100) throw new Error("Invalid customer discount percent");
...
export function validateZSeriesDimensions(item: ZSeriesCatalogItem, width: number, height: number, depth?: number): ZSeriesDimensionError[] {
  ...
  if (height > item.maxHeight) errors.push("height_too_large");
  if (depth == null || !Number.isFinite(depth)) errors.push("depth_required");
  else if (depth <= item.minDepthExclusive) errors.push("depth_too_small");
  return errors;
}
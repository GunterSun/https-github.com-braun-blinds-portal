...
function useRealWorkbookData(){
  const [data,setData]=useState<RealWorkbookData>(()=>{if(typeof window==='undefined')return emptyWorkbookData;try{return JSON.parse(localStorage.getItem('braun-real-workbooks-v4')||'')||emptyWorkbookData}catch{return emptyWorkbookData}});
  const [loading,setLoading]=useState(!data.sources.length);
  useEffect(()=>{if(data.sources.length)return;const run=async()=>{try{const xlsx=await loadXlsxApi();const files=await Promise.all([['jin汇总_已标注汇款.xlsx','/data/jin汇总.xlsx?v=20260723-2'],['批发销售(2).xlsx','/data/批发销售.xlsx?v=20260723-3']].map(async([name,url])=>({name,buffer:await (await fetch(url)).arrayBuffer()})));const parsed=files.map(file=>{const book=xlsx.read(file.buffer);return parseWorkbook(xlsx.utils.sheet_to_json(book.Sheets[book.SheetNames[0]],{header:1,defval:null,raw:true}),file.name)});setData({commerce:parsed.flatMap(x=>x.commerce),wholesale:parsed.flatMap(x=>x.wholesale),retail:parsed.flatMap(x=>x.retail),sources:parsed.map(x=>x.source),updated:new Date().toLocaleString('zh-CN')})}finally{setLoading(false)}};void run()},[]); // eslint-disable-line react-hooks/exhaustive-deps
  return {data,loading};
}
...
function realRowsFor(name:string,data:RealWorkbookData):ModuleRow[]{
  ...
  const retailRows=data.retail.map((r,i)=>({id:r.reference||`RETAIL-${String(i+1).padStart(3,'0')}`,client:r.customer,product:`${r.date||'日期未填'}${r.expense?` · 支出 $${r.expense.toLocaleString()}`:''}`,owner:"批发销售(2)",value:r.salesAmount?`$${r.salesAmount.toLocaleString(undefined,{minimumFractionDigits:2})}`:'—',status:"真实客户订单",tone:"green",date:r.date||"Excel记录"}));
  if(["销售订单","项目管理"].includes(name))return [...retailRows,...orderRows];
  if(["采购订单","供应商账单","收付款"].includes(name))return orderRows;
  if(name==="客户管理")return Array.from(new Set([...data.commerce.map(r=>r.dealer).filter(x=>x&&x!=="未填写"),...data.retail.map(r=>r.customer)])).map((dealer,i)=>{const jin=data.commerce.filter(r=>r.dealer===dealer),retail=data.retail.filter(r=>r.customer===dealer);return{id:`CUSTOMER-${String(i+1).padStart(3,'0')}`,client:dealer,product:`${jin.length+retail.length}笔真实订单`,owner:"Excel导入",value:`$${(jin.reduce((s,r)=>s+r.settlement,0)+retail.reduce((s,r)=>s+r.salesAmount,0)).toLocaleString(undefined,{minimumFractionDigits:2})}`,status:"真实客户",tone:"green",date:"Excel记录"}});
...
function RealOverview({go}:{go:(name:string)=>void}){
  const {data,loading}=useRealWorkbookData(),total=data.commerce.reduce((s,r)=>s+r.settlement,0)+data.retail.reduce((s,r)=>s+r.salesAmount,0),paid=data.commerce.filter(r=>r.remittance.includes('已汇款')).length,dealers=new Set([...data.commerce.map(r=>r.dealer).filter(x=>x&&x!=="未填写"),...data.retail.map(r=>r.customer)]).size;
  const cards=[['真实订单',String(data.commerce.length+data.retail.length),'Jin汇总 + 批发销售(2)'],['订单金额',`$${total.toLocaleString(undefined,{minimumFractionDigits:2})}`,'两份Excel金额列合计'],['客户支出',`$${data.retail.reduce((s,r)=>s+r.expense,0).toLocaleString(undefined,{minimumFractionDigits:2})}`,'批发销售(2)支出列'],['真实客户',String(dealers),'按客户名称去重']];
  return <><section className="hero-card"><div><span className="eyebrow">REAL DATA WORKSPACE / 真实数据工作台</span><h1>Braun Blinds 业务中心</h1><p>只显示真实 Excel 与您手工建立的记录；演示订单已删除。</p></div><div className="hero-actions"><button className="secondary" onClick={()=>go('数据中心')}>更新Excel数据</button><button className="primary" onClick={()=>go('上门测量')}>现场测量</button></div><div className="fabric-orb one"/><div className="fabric-orb two"/></section><section className="kpi-grid">{cards.map(x=><article className="kpi" key={x[0]}><span>{x[0]}</span><strong>{loading?'…':x[1]}</strong><small>{x[2]}</small></article>)}</section><section className="panel real-home"><div className="panel-head"><div><span className="eyebrow">BUSINESS AREAS</span><h2>业务模块状态</h2></div></div><div className="area-grid">{[['客户与销售','客户与订单',`${data.commerce.length+data.retail.length}笔真实订单`],['现场与运营','现场运营中心','等待录入现场记录'],['采购与财务','采购财务中心',`${data.commerce.length}笔CWF + ${data.retail.filter(r=>r.expense).length}笔客户支出`],['分析与管理','真实经营分析','基于真实Excel统计']].map(x=><button key={x[0]} onClick={()=>go(x[1])}><b>{x[0]}</b><span>{x[2]}</span><em>打开 →</em></button>)}</div></section></>;
}
...
function CustomerOrderHub({go}:{go:(name:string)=>void}){
  const {data,loading}=useRealWorkbookData(),[view,setView]=useState<'orders'|'retail'|'customers'>('retail'),[search,setSearch]=useState(''),[paidOnly,setPaidOnly]=useState('全部');
  ...
  const retail=data.retail.filter(r=>`${r.customer}${r.reference}${r.date}`.toLowerCase().includes(search.toLowerCase()));
  const customerNames=Array.from(new Set([...data.commerce.map(r=>r.dealer).filter(x=>x&&x!=='未填写'),...data.retail.map(r=>r.customer)]));
  const customers=customerNames.map(customer=>{const jin=data.commerce.filter(r=>r.dealer===customer),local=data.retail.filter(r=>r.customer===customer);return{customer,orders:jin.length+local.length,total:jin.reduce((s,r)=>s+r.settlement,0)+local.reduce((s,r)=>s+r.salesAmount,0),expense:local.reduce((s,r)=>s+r.expense,0)}}).filter(r=>r.customer.toLowerCase().includes(search.toLowerCase()));
  return <><div className="module-head"><div><span className="eyebrow">REAL CUSTOMERS & ORDERS</span><h1>客户与订单 <small className="en-title">Customers & Orders</small></h1><p>已同时导入 Jin 汇总和《批发销售(2)》中的真实客户、订单金额及客户支出。</p></div><button className="primary" onClick={()=>go('数据中心')}>Excel数据源</button></div><div className="truth-banner"><b>{loading?'正在读取真实文件…':`已读取 ${data.commerce.length+data.retail.length} 笔真实订单、${customers.length} 个客户`}</b><span>来源：{data.sources.join('、')}</span></div><div className="toolbar customer-order-toolbar"><button className={view==='retail'?'selected-filter':''} onClick={()=>setView('retail')}>真实客户订单 ({data.retail.length})</button><button className={view==='orders'?'selected-filter':''} onClick={()=>setView('orders')}>Jin订单 ({data.commerce.length})</button><button className={view==='customers'?'selected-filter':''} onClick={()=>setView('customers')}>客户汇总 ({customers.length})</button><label>⌕ <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="搜索订单、客户或说明…"/></label>{view==='orders'&&<select value={paidOnly} onChange={e=>setPaidOnly(e.target.value)}><option>全部</option><option>已汇款</option><option>未汇款</option></select>}</div><div className="panel module-panel"><div className="table-wrap">{view==='retail'?<table><thead><tr><th>日期</th><th>真实客户</th><th>订单号 / 说明</th><th>订单金额</th><th>支出</th><th>来源</th></tr></thead><tbody>{retail.map((r,i)=><tr key={`${r.customer}-${r.reference}-${i}`}><td>{r.date||'—'}</td><td><b>{r.customer}</b></td><td>{r.reference||'—'}</td><td>{r.salesAmount?`$${r.salesAmount.toLocaleString(undefined,{minimumFractionDigits:2})}`:'—'}</td><td>{r.expense?`$${r.expense.toLocaleString(undefined,{minimumFractionDigits:2})}`:'—'}</td><td>批发销售(2).xlsx</td></tr>)}</tbody></table>:view==='orders'?<table><thead><tr><th>PO号</th><th>客户 Dealer</th><th>项目代号 Sidemark</th><th>产品</th><th>数量</th><th>结算金额</th><th>已到账</th><th>汇款</th></tr></thead><tbody>{orders.map((r,i)=><tr key={`${r.po}-${i}`}><td><b>{r.po}</b></td><td>{r.dealer}</td><td>{r.sidemark}</td><td>{r.product}</td><td>{r.qty||'—'}</td><td>${r.settlement.toLocaleString(undefined,{minimumFractionDigits:2})}</td><td>{r.received?`$${r.received.toLocaleString(undefined,{minimumFractionDigits:2})}`:'—'}</td><td><Status tone={r.remittance.includes('已汇款')?'green':'amber'}>{r.remittance}</Status></td></tr>)}</tbody></table>:<table><thead><tr><th>真实客户</th><th>订单数</th><th>订单金额</th><th>客户支出</th><th>数据状态</th></tr></thead><tbody>{customers.map(r=><tr key={r.customer}><td><b>{r.customer}</b></td><td>{r.orders}</td><td>${r.total.toLocaleString(undefined,{minimumFractionDigits:2})}</td><td>{r.expense?`$${r.expense.toLocaleString(undefined,{minimumFractionDigits:2})}`:'—'}</td><td><Status tone="green">真实Excel</Status></td></tr>)}</tbody></table>}</div></div></>;
}
...
type KtExpense={id:number;date:string;payee:string;description:string;usd:number;rmb:number;remarks:string};
const ktExpenses:KtExpense[]=[
  {id:1,date:"2026-06-28",payee:"李帅 (Li Shuai)",description:"墙安全扣 (Wallpaper deduction)",usd:0,rmb:640,remarks:"200P"},
  {id:2,date:"",payee:"张卫国",description:"弹簧系统",usd:0,rmb:800,remarks:"72B1, 72B2, 96b2，各20个"},
  {id:3,date:"",payee:"样板 (Samples)",description:"样板费 (Samples)",usd:0,rmb:600,remarks:""},
  {id:4,date:"2026-05-14",payee:"赵 (Zhao)",description:"",usd:0,rmb:1386,remarks:""},
  {id:5,date:"2026-04-02",payee:"赵 (Zhao)",description:"",usd:0,rmb:3352,remarks:""},
  {id:6,date:"2026-06-05",payee:"赵 (Zhao)",description:"",usd:0,rmb:500,remarks:""},
  {id:7,date:"2026-05-20",payee:"赵 (Zhao)",description:"",usd:0,rmb:544,remarks:""},
  {id:8,date:"2026-05-21",payee:"赵 (Zhao)",description:"",usd:0,rmb:6800,remarks:""},
  {id:9,date:"2026-05-14",payee:"赵 (Zhao)",description:"",usd:0,rmb:3064,remarks:""},
  {id:10,date:"2026-05-04",payee:"赵 (Zhao)",description:"",usd:0,rmb:1492,remarks:""},
  {id:11,date:"",payee:"美国转 (US Transfer)",description:"美金转账 (US Wire)",usd:32040,rmb:0,remarks:"美国转 $32040"},
  {id:12,date:"2026-03-16",payee:"人民币转账 (RMB Transfer)",description:"网银转账 1 (RMB Transfer 1)",usd:0,rmb:50000,remarks:"人民币转账"},
  {id:13,date:"2026-04-12",payee:"人民币转账 (RMB Transfer)",description:"网银转账 2 (RMB Transfer 2)",usd:0,rmb:50000,remarks:"人民币转账"},
  {id:14,date:"2026-05-05",payee:"人民币转账 (RMB Transfer)",description:"网银转账 3 (RMB Transfer 3)",usd:0,rmb:50000,remarks:"人民币转账 t/s"},
  {id:15,date:"",payee:"东升 (Dongsheng)",description:"存款",usd:0,rmb:3173,remarks:"东升 余款"},
  {id:16,date:"",payee:"Jin",description:"吃饭 (Meal/Dining)",usd:0,rmb:2000,remarks:"吃饭"}
];

function PurchasingFinanceCenter(){
  const {data,loading}=useRealWorkbookData(),[tab,setTab]=useState<'orders'|'reconcile'|'remittance'|'expenses'|'clientExpenses'>('orders'),[search,setSearch]=useState('');
  ...
  const expenseRows=ktExpenses.filter(r=>`${r.date}${r.payee}${r.description}${r.remarks}`.toLowerCase().includes(search.toLowerCase())),usdTotal=expenseRows.reduce((s,r)=>s+r.usd,0),rmbTotal=expenseRows.reduce((s,r)=>s+r.rmb,0);
  const clientExpenses=data.retail.filter(r=>r.expense&&`${r.customer}${r.reference}${r.date}`.toLowerCase().includes(search.toLowerCase())),clientExpenseTotal=clientExpenses.reduce((s,r)=>s+r.expense,0);
  const showingExpenses=tab==='expenses'||tab==='clientExpenses';
  return <><div className="module-head"><div><span className="eyebrow">CWF RECONCILIATION / PURCHASING & FINANCE</span><h1>采购财务中心 <small className="en-title">Purchasing & Finance</small></h1><p>KT支出项目、真实客户支出和Jin订单对账分别记录，不混为通用支出。</p></div><a className="primary" href="/data/KT支出明细.xlsx?v=20260723" download>下载KT支出明细</a></div>
  <div className="finance-note"><b>重要：</b>{tab==='expenses'?'16条记录统一归入“KT支出项目”，美元和人民币分别汇总。':tab==='clientExpenses'?'客户支出来自《批发销售(2)》支出列，并关联真实客户与订单。':'“已到账金额”有些是多个PO的合并汇款，系统保留原表金额和组合说明，不自动分摊。'}</div>
  <section className="field-kpis">{tab==='expenses'?<><div><span>KT支出记录</span><b>{expenseRows.length}</b></div><div><span>美元支出</span><b>${usdTotal.toLocaleString(undefined,{minimumFractionDigits:2})}</b></div><div><span>人民币支出</span><b>¥{rmbTotal.toLocaleString()}</b></div><div><span>项目名称</span><b>KT支出项目</b></div></>:tab==='clientExpenses'?<><div><span>客户支出记录</span><b>{clientExpenses.length}</b></div><div><span>客户支出合计</span><b>${clientExpenseTotal.toLocaleString(undefined,{minimumFractionDigits:2})}</b></div><div><span>关联客户</span><b>{new Set(clientExpenses.map(r=>r.customer)).size}</b></div><div><span>数据来源</span><b>批发销售(2)</b></div></>:<><div><span>CWF订单</span><b>{loading?'…':rows.length}</b></div><div><span>结算金额合计</span><b>${total.toLocaleString(undefined,{maximumFractionDigits:2})}</b></div><div><span>以前发邮件合计</span><b>${previous.toLocaleString(undefined,{maximumFractionDigits:2})}</b></div><div><span>表内已到账合计</span><b>${received.toLocaleString(undefined,{maximumFractionDigits:2})}</b></div></>}</section>
  <div className="toolbar finance-tabs"><button className={tab==='orders'?'selected-filter':''} onClick={()=>setTab('orders')}>采购订单</button><button className={tab==='reconcile'?'selected-filter':''} onClick={()=>setTab('reconcile')}>对账金额</button><button className={tab==='remittance'?'selected-filter':''} onClick={()=>setTab('remittance')}>汇款状态 ({paid}/{rows.length})</button><button className={tab==='expenses'?'selected-filter':''} onClick={()=>setTab('expenses')}>KT支出项目 ({ktExpenses.length})</button><button className={tab==='clientExpenses'?'selected-filter':''} onClick={()=>setTab('clientExpenses')}>客户支出 ({data.retail.filter(r=>r.expense).length})</button><label>⌕ <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={showingExpenses?"搜索日期、客户、订单或说明…":"搜索PO、客户、汇款组合…"}/></label></div>
  <div className="panel module-panel"><div className="table-wrap">{tab==='expenses'?<table><thead><tr><th>项目</th><th>日期</th><th>付款对象</th><th>说明/品类</th><th>美元</th><th>人民币</th><th>备注</th></tr></thead><tbody>{expenseRows.map(r=><tr key={r.id}><td><b>KT支出项目</b></td><td>{r.date||'未填写'}</td><td>{r.payee}</td><td>{r.description||'—'}</td><td>{r.usd?`$${r.usd.toLocaleString(undefined,{minimumFractionDigits:2})}`:'—'}</td><td>{r.rmb?`¥${r.rmb.toLocaleString()}`:'—'}</td><td>{r.remarks||'—'}</td></tr>)}</tbody></table>:tab==='clientExpenses'?<table><thead><tr><th>日期</th><th>真实客户</th><th>关联订单 / 说明</th><th>支出金额</th><th>来源</th></tr></thead><tbody>{clientExpenses.map((r,i)=><tr key={`${r.customer}-${r.reference}-${i}`}><td>{r.date||'—'}</td><td><b>{r.customer}</b></td><td>{r.reference||'—'}</td><td>${r.expense.toLocaleString(undefined,{minimumFractionDigits:2})}</td><td>批发销售(2).xlsx</td></tr>)}</tbody></table>:tab==='orders'?<table><thead><tr><th>PO号</th><th>Dealer</th><th>Sidemark</th><th>产品</th><th>数量</th><th>结算金额</th></tr></thead><tbody>{rows.map((r,i)=><tr key={`${r.po}-${i}`}><td><b>{r.po}</b></td><td>{r.dealer}</td><td>{r.sidemark}</td><td>{r.product}</td><td>{r.qty||'—'}</td><td>${r.settlement.toLocaleString(undefined,{minimumFractionDigits:2})}</td></tr>)}</tbody></table>:tab==='reconcile'?<table><thead><tr><th>PO号</th><th>以前发邮件</th><th>汇款组合/说明</th><th>已到账金额</th><th>结算金额</th><th>原表差额说明</th></tr></thead><tbody>{rows.map((r,i)=><tr key={`${r.po}-${i}`}><td><b>{r.po}</b></td><td>{r.previousEmail?`$${r.previousEmail.toLocaleString(undefined,{minimumFractionDigits:2})}`:'—'}</td><td>{r.transferReference||'—'}</td><td>{r.received?`$${r.received.toLocaleString(undefined,{minimumFractionDigits:2})}`:'—'}</td><td>${r.settlement.toLocaleString(undefined,{minimumFractionDigits:2})}</td><td>{r.difference||'—'}</td></tr>)}</tbody></table>:<table><thead><tr><th>PO号</th><th>Dealer</th><th>结算金额</th><th>汇款状态</th><th>核对提示</th></tr></thead><tbody>{rows.map((r,i)=><tr key={`${r.po}-${i}`}><td><b>{r.po}</b></td><td>{r.dealer}</td><td>${r.settlement.toLocaleString(undefined,{minimumFractionDigits:2})}</td><td><Status tone={r.remittance.includes('已汇款')?'green':'amber'}>{r.remittance}</Status></td><td>{r.transferReference?'组合汇款，需按说明核对':r.received&&Math.abs(r.received-r.settlement)>.02?'到账与结算不同':'—'}</td></tr>)}</tbody></table>}</div></div></>;
}
...
type RetailRow={customer:string;date:string;reference:string;salesAmount:number;expense:number};
type RealWorkbookData={commerce:CommerceRow[];wholesale:WholesaleRow[];retail:RetailRow[];sources:string[];updated:string};
const emptyWorkbookData:RealWorkbookData={commerce:[],wholesale:[],retail:[],sources:[],updated:""};
...
const excelDate=(v:SheetCell)=>{if(typeof v==='number'){const date=new Date(Date.UTC(1899,11,30)+v*86400000);return date.toISOString().slice(0,10)}const text=cell(v);if(!text)return'';const date=new Date(text);return Number.isNaN(date.getTime())?text:date.toISOString().slice(0,10)};
function parseWorkbook(rows:SheetCell[][],source:string){
  if(rows.some(r=>r.some(v=>cell(v).includes('客户名字 (Dealer)')))){
    ...
    return {commerce,wholesale:[] as WholesaleRow[],retail:[] as RetailRow[],source};
  }
  if(source.includes('批发销售')&&rows.some(r=>cell(r[4])==='支出')){
    const retail=rows.slice(1).filter(r=>cell(r[0])&&(cell(r[2])||money(r[3])||money(r[4]))).map(r=>({customer:cell(r[0]),date:excelDate(r[1]),reference:cell(r[2]),salesAmount:money(r[3]),expense:money(r[4])}));
    return {commerce:[] as CommerceRow[],wholesale:[] as WholesaleRow[],retail,source};
  }
  ...
  return {commerce:[] as CommerceRow[],wholesale,retail:[] as RetailRow[],source};
}

function DataCenterPage(){
  const [data,setData]=useState<RealWorkbookData>(()=>{if(typeof window==='undefined')return emptyWorkbookData;try{return JSON.parse(localStorage.getItem('braun-real-workbooks-v4')||'')||emptyWorkbookData}catch{return emptyWorkbookData}}),[tab,setTab]=useState<'commerce'|'wholesale'|'retail'>('retail'),[search,setSearch]=useState(''),[busy,setBusy]=useState(()=>typeof window==='undefined'||!localStorage.getItem('braun-real-workbooks-v4')),[error,setError]=useState('');
  const readFiles=async(files:Array<{name:string;buffer:ArrayBuffer}>,persist=true)=>{setBusy(true);setError('');try{const xlsx=await loadXlsxApi();const parsed=files.map(file=>{const book=xlsx.read(file.buffer);const sheet=book.Sheets[book.SheetNames[0]];return parseWorkbook(xlsx.utils.sheet_to_json(sheet,{header:1,defval:null,raw:true}),file.name)});const next:RealWorkbookData={commerce:parsed.flatMap(x=>x.commerce),wholesale:parsed.flatMap(x=>x.wholesale),retail:parsed.flatMap(x=>x.retail),sources:parsed.map(x=>x.source),updated:new Date().toLocaleString('zh-CN')};setData(next);if(persist)localStorage.setItem('braun-real-workbooks-v4',JSON.stringify(next))}catch{setError('无法读取文件，请确认它是有效的 .xlsx 文件。')}finally{setBusy(false)}};
  const loadBundled=async()=>{const files=await Promise.all([['jin汇总_已标注汇款.xlsx','/data/jin汇总.xlsx?v=20260723-2'],['批发销售(2).xlsx','/data/批发销售.xlsx?v=20260723-3']].map(async([name,url])=>({name,buffer:await (await fetch(url)).arrayBuffer()})));await readFiles(files,false)};
  ...
  <div className="toolbar data-toolbar"><button className={tab==='retail'?'selected-filter':''} onClick={()=>setTab('retail')}>真实客户订单 ({data.retail.length})</button><button className={tab==='commerce'?'selected-filter':''} onClick={()=>setTab('commerce')}>Commerce订单 ({data.commerce.length})</button><button className={tab==='wholesale'?'selected-filter':''} onClick={()=>setTab('wholesale')}>其他批发 ({data.wholesale.length})</button><label>⌕ <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="搜索PO、客户、项目、产品…"/></label></div>
  <div className="panel module-panel real-data-table"><div className="table-wrap">{busy?<div className="empty-search">正在读取真实 Excel 数据…</div>:tab==='retail'?<table><thead><tr><th>日期</th><th>真实客户</th><th>订单号 / 说明</th><th>订单金额</th><th>支出</th></tr></thead><tbody>{retail.map((r,i)=><tr key={`${r.customer}-${r.reference}-${i}`}><td>{r.date||'—'}</td><td><b>{r.customer}</b></td><td>{r.reference||'—'}</td><td>{r.salesAmount?`$${r.salesAmount.toLocaleString(undefined,{minimumFractionDigits:2})}`:'—'}</td><td>{r.expense?`$${r.expense.toLocaleString(undefined,{minimumFractionDigits:2})}`:'—'}</td></tr>)}</tbody></table>:tab==='commerce'?<table><thead><tr><th>PO号</th><th>客户 / Dealer</th><th>项目代号</th><th>产品</th><th>数量</th><th>结算金额</th><th>已到账</th><th>汇款状态</th></tr></thead><tbody>{commerce.map((r,i)=><tr key={`${r.po}-${i}`}><td><b>{r.po}</b></td><td>{r.dealer}</td><td>{r.sidemark}</td><td>{r.product}</td><td>{r.qty||'—'}</td><td>${r.settlement.toLocaleString(undefined,{minimumFractionDigits:2})}</td><td>{r.received?`$${r.received.toLocaleString(undefined,{minimumFractionDigits:2})}`:'—'}</td><td><Status tone={r.remittance.includes('已汇款')?'green':'amber'}>{r.remittance}</Status></td></tr>)}</tbody></table>:<table><thead><tr><th>销售/来源</th><th>日期</th><th>订单或说明</th><th>金额</th></tr></thead><tbody>{wholesale.map((r,i)=><tr key={`${r.seller}-${i}`}><td><b>{r.seller}</b></td><td>{r.date||'—'}</td><td>{r.reference||'—'}</td><td>{r.amount?`$${r.amount.toLocaleString(undefined,{minimumFractionDigits:2})}`:'—'}</td></tr>)}</tbody></table>}</div></div></>;
}

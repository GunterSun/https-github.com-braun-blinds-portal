...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    if(action==="save_measurement"){
      ...
      const latest=await db.select({version:max(measurementVersions.version)}).from(measurementVersions).where(eq(measurementVersions.windowId,windowId)),version=Number(latest[0]?.version||0)+1;

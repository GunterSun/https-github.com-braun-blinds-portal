...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    if(action==="save_measurement"){
      const windowId=Number(body.windowId),window=(await db.select().from(measureWindows).where(and(eq(measureWindows.id,windowId),eq(measureWindows.propertyId,propertyId))).limit(1))[0],productType=clean(body.productType,40),mountType=clean(body.mountType,30)||"inside",rawValues=Array.isArray(body.values)?body.values:[];if(!window||!productTypes.has(productType)||!["inside","outside","ceiling"].includes(mountType))return NextResponse.json({error:"Window、产品或安装方式无效 / Invalid window, product or mount"},{status:400});
      ...
      const present=new Set(values.map(value=>value.fieldKey)),missing=requiredMeasureFields(productType,mountType).filter(key=>!present.has(key));if(missing.length)return NextResponse.json({error:`产品必填尺寸缺失 / Required product dimensions missing: ${missing.join(", ")}`,code:"PRODUCT_MEASUREMENTS_MISSING",missingFields:missing},{status:400});
      const obstacleNotes=clean(body.obstacleNotes,2000),notes=clean(body.notes,2000);if(requiresPowerConfirmation(productType)&&!`${obstacleNotes} ${notes}`.match(/power|outlet|charger|电源|插座|充电/i))return NextResponse.json({error:"电动产品必须记录电源或插座确认 / Motorized products require power or outlet confirmation",code:"POWER_CONFIRMATION_REQUIRED"},{status:400});
      ...
      const created=await db.insert(measurementVersions).values({windowId,version,productType,mountType,controlSide:clean(body.controlSide,30)||"unspecified",status:"draft",obstacleNotes,notes,createdBy:user.id}).returning();

...
export const customerQuoteVersions = sqliteTable("customer_quote_versions", {
  ...
  status: text("status").notNull().default("issued"), currency: text("currency").notNull().default("USD"), subtotal: real("subtotal").notNull(), discountAmount: real("discount_amount").notNull().default(0), taxAmount: real("tax_amount").notNull().default(0), installationFee: real("installation_fee").notNull().default(0), shippingFee: real("shipping_fee").notNull().default(0), depositRequired: real("deposit_required").notNull().default(0), total: real("total").notNull(),
...
export const customerQuoteShipping=sqliteTable("customer_quote_shipping",{
  id:integer("id").primaryKey({autoIncrement:true}),quoteVersionId:integer("quote_version_id").notNull().unique(),amount:real("amount").notNull().default(0),currency:text("currency").notNull().default("USD"),labelZh:text("label_zh").notNull().default("运输费"),labelEn:text("label_en").notNull().default("Shipping"),createdAt:text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
});

export const zSeriesQuoteIssuanceOperations=sqliteTable("z_series_quote_issuance_operations",{
  id:integer("id").primaryKey({autoIncrement:true}),idempotencyKey:text("idempotency_key").notNull().unique(),requestSha256:text("request_sha256").notNull(),quoteVersionId:integer("quote_version_id"),createdAt:text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
});

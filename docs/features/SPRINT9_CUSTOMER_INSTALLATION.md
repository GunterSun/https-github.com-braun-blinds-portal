...
Installation evidence images are uploaded as JPEG, PNG or WebP (maximum 10 MB) to the private `MEDIA` R2 binding. Access is re-authorized against the installation on every read; D1 stores only the protected media URL. After immutable signature, an authenticated endpoint generates a bilingual PDF containing the Order, project, address, completed Room/Window list, exceptions, signer, signed time and evidence SHA-256.

...
function initApp() {
    ...
    // Braun Measure transfer file
    setupMeasurementImport();

    // Load initial materials table
    renderMaterialsTable();
}

function setupMeasurementImport() {
    const button = document.getElementById('btn-import-measure');
    const input = document.getElementById('measure-file-input');
    ...
    input.addEventListener('change', event => {
        ...
        reader.onload = () => {
            try {
                const transfer = JSON.parse(String(reader.result || ''));
                importMeasurementTransfer(transfer);
            } catch (error) {
                alert('测量文件无法读取，请确认文件来自 Braun 测量系统。\nInvalid Braun measurement file.');
            }
            input.value = '';
        };
        reader.readAsText(file);
    });
    const pending = localStorage.getItem('braun-measure-transfer-pending');
    if (pending) {
        try {
            importMeasurementTransfer(JSON.parse(pending));
            localStorage.removeItem('braun-measure-transfer-pending');
        } catch (error) {
            console.error('Pending measurement import failed', error);
        }
    }
}

function importMeasurementTransfer(transfer) {
    if (!transfer || transfer.format !== 'braun-measure-transfer' || !transfer.project || !Array.isArray(transfer.project.rooms)) {
        throw new Error('Invalid measurement transfer');
    }
    const project = transfer.project;
    const openings = [];
    project.rooms.forEach(room => (room.openings || []).forEach(opening => openings.push({ room, opening })));
    if (!openings.length) return alert('测量文件中没有窗户数据。 / No window measurements found.');
    if (currentOrderItems.length && !confirm(`当前已有 ${currentOrderItems.length} 项。确定用测量数据替换吗？\nReplace current items with imported measurements?`)) return;
    const toInches = value => {
        const number = Number(value) || 0;
        return project.unit === 'mm' ? number / 25.4 : number;
    };
    const pick = (opening, keys, mode) => {
        const values = keys.map(k => Number(opening[k])).filter(v => Number.isFinite(v) && v > 0);
        if (!values.length) return 0;
        return mode === 'outside' ? Math.max(...values) : Math.min(...values);
    };
    const productType = name => {
        const text = String(name || '').toLowerCase();
        if (text.includes('drapery') || text.includes('窗帘')) return 'drapery';
        if (text.includes('track') || text.includes('rod') || text.includes('轨道') || text.includes('帘杆')) return 'rod';
        return 'roman';
    };
    currentOrderItems = openings.map(({room, opening}, index) => {
        const type = productType(opening.product);
        const width = toInches(pick(opening, ['wt','wm','wb'], opening.mount));
        const height = type === 'rod' ? 0 : toInches(pick(opening, ['hl','hc','hr'], opening.mount));
        const depth = toInches(opening.depth);
        const rawW = [opening.wt, opening.wm, opening.wb].filter(Boolean).join('/');
        const rawH = [opening.hl, opening.hc, opening.hr].filter(Boolean).join('/');
        const notes = [
            `Imported from Braun Measure: ${opening.product || ''}`,
            `Opening: ${opening.name || ''}`,
            `Depth: ${depth ? depth.toFixed(3) + ' in' : 'N/A'}`,
            `Raw W: ${rawW || 'N/A'} ${project.unit}; Raw H: ${rawH || 'N/A'} ${project.unit}`,
            opening.notes || ''
        ].filter(Boolean).join(' | ');
        const item = {
            number: index + 1, room: `${room.name || 'Room'} - ${opening.name || `Window ${index + 1}`}`,
            type, qty: 1, width, width_raw: String(width), height, height_raw: String(height),
            fabric_code: '', fabric_group: 'H', special_instructions: notes,
            mount: opening.mount === 'outside' ? 'OB' : 'IM', image: opening.photo || '',
            measurement_depth: depth, measurement_source: 'Braun Measure',
            shade_style: 'plain', lift_control: 'cordless', valance: 'no', accessories: 'no', lining: 'standard', trim: 'no',
            header_style: 'double_pinch', panel_type: 'single', track: 'no', pleat_upgrade: 'no', tieband: 'no', memory_shaping: 'no'
        };
        item.pricing = calculateItemPrice(item);
        return item;
    });
    document.getElementById('meta-customer').value = project.customer || '';
    document.getElementById('meta-po').value = project.order || '';
    document.getElementById('meta-phone').value = project.phone || '';
    document.getElementById('meta-address').value = project.address || '';
    renderItemsTable();
    alert(`已连接 Braun 测量系统：成功导入 ${currentOrderItems.length} 扇窗。\nImported ${currentOrderItems.length} measured openings.`);
}
...
const JIN_ROMAN_BASE_PRICES = {
    'cordless': {
        ...
        'balloon': { 'A': 10.00, 'B': 10.00, 'C': 10.00, 'D': 10.00 }
    },
    'ccl': {
...
function calculateRomanCording(width_cm, shadeStyle) {
    // 车间规则（2026-07 修订）：总宽≤50cm边距8cm；<190cm边距10cm；≥190cm边距15cm。
    let cords = 2;
    let edge = width_cm <= 50 ? 8 : (width_cm < 190 ? 10 : 15);
    if (shadeStyle === 'tdbu') {
        if (width_cm <= 80) cords = 2;
        else if (width_cm <= 130) cords = 3;
        else if (width_cm <= 180) cords = 4;
        else cords = 5;
    } else {
        if (width_cm <= 65) cords = 2;
        else if (width_cm <= 110) cords = 3;
        else if (width_cm <= 160) cords = 4;
        else cords = 5;
    }
    return { cords, edge };
}
...
function calculateFabricationDetails(item) {
    if (item.type === 'roman') {
        const isIB = (item.mount === 'IM' || item.mount === 'IB');
        // IB（口内）成品宽自动减 1/4"（约 6mm）；OB（口外）不减。
        const orderedWIn = Number(item.width) || 0;
        const widthDeductionIn = isIB ? 0.25 : 0;
        const w_in_finished = Math.max(0, orderedWIn - widthDeductionIn);
        let ordered_w_cm = (orderedWIn * 2.54).round(1);
        ...
        let cut_w = (w_cm + 10).round(1); // 左右各 5cm 包边折量
        return {
            type: 'roman', ordered_w_cm, width_deduction_in: widthDeductionIn,
            width_deduction_mm: isIB ? 6 : 0, w_in_finished, w_cm, h_cm, cut_w,
            cut_h: draw.cut_h, folds, marks: draw.marks, marksStr: draw.marks.join(', '),
            cords: cording.cords, edge: cording.edge,
            cordStr: cording.cords + '根 (Cords) | 边距 Edge: ' + cording.edge + ' cm',
            D: draw.D, H: draw.H,
            foldGuideline: sheetName === '平铺款无明线'
                ? 'Flat B 平铺款分档约 35cm'
                : '鱼鳞/前凸/后凸款分档控制在 60cm 以内，通常约 50cm'
        };
...
function calculateItemPrice(item) {
    ...
    if (item.type === 'roman') {
        ...
        if (h > 104 && h <= 132) {
            ...
            // “上浮200%” means adding 200% to the original base rate,
            // so the height-adjusted base rate becomes 300% of the original.
            heightPct = 2.00;
            attributes.push('Height Surcharge 238-278" (+200%)');
        }
...
function parseExcelOrderFile(file) {
    ...
    reader.onload = function(e) {
        ...
        for (let r = headerRowIdx + 1; r <= range.e.r; r++) {
            ...
            // Each order row represents exactly ONE finished product:
            // either a regular fabric drapery OR a sheer drapery.
            // The first Pattern/Fabric column is the supplier material code;
            // the second Pattern/Fabric column is the material name / collection code,
            // not a second layer. Never split one row into two invoice items.
            const rawFabricCode = colMap.fabric !== undefined ? String(getCellValue(sheet, r, colMap.fabric)).trim() : '';
            const rawPatternName = colMap.pattern_name !== undefined ? String(getCellValue(sheet, r, colMap.pattern_name)).trim() : '';

            let fabricCode = rawFabricCode || rawPatternName;
            let fabricGroup = 'H';
            const matchedMat = findMaterialInDb(rawFabricCode, rawPatternName);
            if (matchedMat) {
                fabricGroup = matchedMat.group;
                fabricCode = matchedMat.code || matchedMat.pattern_code || fabricCode;
            } else {
                const ext = extractFabricGroup(rawFabricCode, rawPatternName);
                if (ext) fabricGroup = ext;
            }

            const combinedFabricText = `${rawFabricCode} ${rawPatternName} ${fabricCode}`.toUpperCase();
            const rowIsSheer = isDraperySheer(rawFabricCode) ||
                isDraperySheer(rawPatternName) ||
                isDraperySheer(fabricCode) ||
                combinedFabricText.includes('SHEER') ||
                combinedFabricText.includes('纱');
            ...
            let itemsToImport = [{
                ...
                fabric_code: fabricCode,
                fabric_name: rawPatternName,
                source_fabric_code: rawFabricCode,
                fabric_group: fabricGroup,
                mount: mount.toUpperCase().includes('OM') || mount.toUpperCase().includes('OB') ? 'OM' : 'IM',
                special_instructions: special || notes,
                is_sheer_force_sheer: rowIsSheer,
                is_sheer_force_regular: !rowIsSheer
            }];
...
function setupActionButtons() {
    ...
    const longInvoiceBtn = document.getElementById('btn-export-long-invoice');
    if (longInvoiceBtn) {
        longInvoiceBtn.addEventListener('click', () => {
            if (currentOrderItems.length === 0) {
                alert('报价细目表为空！');
                return;
            }
            exportInvoiceLongPdf();
        });
    }
...
function sendEmailNotification() {
    ...
    let subject = `BRAUN INTERNATIONAL LLC Invoice: PO ${po} - ${customer}`;
        ...
        <div class="print-invoice-sheet">
            ...
            <div class="print-invoice-footer-container">
                <div class="print-terms-box">
                    <p><strong>Terms and Conditions (BRAUN INTERNATIONAL LLC):</strong></p>
                    ...
                    <p>4. Draperies are calculated on finished width (min 20" finished width). Height surcharges: 104-132" +20%, 132-158" +50%, 158-198" +100%, 198-238" +150%, and over 238" up to 278" +200%.</p>
                </div>
...
// Export Invoice as a single-page long-image PDF (no blank first page, no pagination)
function exportInvoiceLongPdf() {
    try {
        const canvas = buildInvoiceLongCanvas();
        const jpegDataUrl = canvas.toDataURL('image/jpeg', 0.94);
        const filenameBase = sanitizeFileName((document.getElementById('meta-po').value || 'Invoice') + '_Invoice_Long');
        downloadCanvasAsSinglePagePdf(jpegDataUrl, canvas.width, canvas.height, filenameBase + '.pdf');
    } catch (error) {
        console.error(error);
        alert('生成长图 PDF 失败：' + (error.message || error));
    }
}

function sanitizeFileName(name) {
    return String(name || 'Invoice').replace(/[\\/:*?"<>|]+/g, '_').trim() || 'Invoice';
}
...
function wrapCanvasText(ctx, text, maxWidth) {
    ...
    paragraphs.forEach((paragraph, pIndex) => {
        ...
        let current = '';
        const tokens = paragraph.split(/(\s+|(?=[\u4e00-\u9fff])|(?<=[\u4e00-\u9fff]))/).filter(Boolean);
        ...
        if (current || pIndex === paragraphs.length - 1) lines.push(current.trimEnd());
    });
    return lines.length ? lines : [''];
}

function drawWrappedCellText(ctx, text, x, y, width, lineHeight, maxLines, align = 'left', bold = false) {
    ctx.font = `${bold ? '600 ' : ''}22px Arial, sans-serif`;
    ctx.fillStyle = '#111';
    ctx.textBaseline = 'top';
    const lines = wrapCanvasText(ctx, text, Math.max(10, width - 16));
    const visible = lines.slice(0, maxLines);
    visible.forEach((line, idx) => {
        let tx = x + 8;
        if (align === 'center') tx = x + width / 2;
        if (align === 'right') tx = x + width - 8;
        ctx.textAlign = align;
        ctx.fillText(line, tx, y + 7 + idx * lineHeight);
    });
    ctx.textAlign = 'left';
}

function buildInvoiceLongCanvas() {
    const customer = document.getElementById('meta-customer').value || 'N/A';
    const po = document.getElementById('meta-po').value || 'N/A';
    const sidemark = document.getElementById('meta-sidemark').value || 'N/A';
    const phone = document.getElementById('meta-phone').value || 'N/A';
    ...
    const rowH = 112;
    const totalsRows = 5 + (additionVal > 0 ? 1 : 0) + (deductionVal > 0 ? 1 : 0);
    const totalsH = totalsRows * 54 + 40;
    const termsH = 310;
    const signatureH = 120;
    const H = margin * 2 + headerH + metaH + tableHeaderH + currentOrderItems.length * rowH + totalsH + termsH + signatureH;

    const canvas = document.createElement('canvas');
    canvas.width = W;
    canvas.height = Math.max(H, 1000);
    ...
    ctx.textBaseline = 'top';
    ctx.fillText('BRAUN INTERNATIONAL LLC', margin, y);
    ctx.fillStyle = '#111';
    ctx.font = '700 27px Arial, sans-serif';
    ctx.textAlign = 'right';
    ctx.fillText('PROFORMA INVOICE FORM', W - margin, y + 18);
    ctx.textAlign = 'left';
    y += 76;
    ctx.strokeStyle = '#B55A30';
    ctx.lineWidth = 5;
    ctx.beginPath(); ctx.moveTo(margin, y); ctx.lineTo(W - margin, y); ctx.stroke();
    y += 35;

    const meta = [
        ['Customer (Dealer)', customer], ['Invoice / PO#', po],
        ['Sidemark / Project', sidemark], ['Order Date', new Date().toLocaleDateString('zh-CN')],
        ['Phone Number', phone], ['Shipping Address', address]
    ];
    const metaColW = (W - margin * 2) / 2;
    ctx.font = '22px Arial, sans-serif';
    meta.forEach((m, i) => {
        const col = i % 2, row = Math.floor(i / 2);
        const x = margin + col * metaColW;
        const yy = y + row * 55;
        ctx.font = '700 22px Arial, sans-serif'; ctx.fillStyle = '#222'; ctx.fillText(m[0] + ':', x, yy);
        ctx.font = '22px Arial, sans-serif';
        const labelW = ctx.measureText(m[0] + ':').width + 14;
        const lines = wrapCanvasText(ctx, m[1], metaColW - labelW - 20).slice(0, 2);
        lines.forEach((line, li) => ctx.fillText(line, x + labelW, yy + li * 25));
    });
    y += metaH;

    const widths = [70, 170, 65, 145, 165, 230, 330, 120, 130, 235];
    const headers = ['NO','Room','Qty','Type','Size (W×H)','Product','Description','Rate','Amount','Instructions'];
    let x = margin;
    ctx.fillStyle = '#B55A30'; ctx.fillRect(margin, y, W - margin*2, tableHeaderH);
    headers.forEach((h, i) => {
        ctx.fillStyle = '#fff'; ctx.font = '700 21px Arial, sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
        ctx.fillText(h, x + widths[i]/2, y + tableHeaderH/2);
        x += widths[i];
    });
    ctx.textAlign='left'; ctx.textBaseline='top';
    y += tableHeaderH;

    let subtotal = 0, rodCost = 0, oversizedCount = 0;
    currentOrderItems.forEach((item, idx) => {
        const typeLabel = item.type === 'roman' ? 'Roman Shade' : (item.type === 'drapery' ? 'Drapery' : (item.type === 'manual' ? 'Adjustment' : 'Hardware'));
        const sizeLabel = item.type === 'manual' ? '-' : (item.type === 'rod' ? item.width_raw + '"' : item.width_raw + '" × ' + item.height_raw + '"');
        let productDesc = item.fabric_code || 'N/A';
        if (item.type !== 'rod' && item.type !== 'manual') productDesc += ' (' + item.lining + ')';
        else if (item.type === 'manual') productDesc = 'Adjustment';
        let instructions = item.special_instructions || '';
        if (item.mount && item.type !== 'rod' && item.type !== 'manual') instructions = '[' + item.mount + '] ' + instructions;
        const vals = [idx+1,item.room,item.qty,typeLabel,sizeLabel,productDesc,item.pricing.desc,'$'+item.pricing.rate.toFixed(2),'$'+item.pricing.amount.toFixed(2),instructions];
        x = margin;
        ctx.fillStyle = idx % 2 ? '#FAFAFA' : '#FFFFFF'; ctx.fillRect(margin,y,W-margin*2,rowH);
        ctx.strokeStyle='#333'; ctx.lineWidth=1;
        vals.forEach((val,i)=>{
            ctx.strokeRect(x,y,widths[i],rowH);
            const align = [0,2,7,8].includes(i) ? 'center' : 'left';
            drawWrappedCellText(ctx,val,x,y,widths[i],27,3,align,i===1);
            x += widths[i];
        });
        y += rowH;
        if (item.type === 'rod') rodCost += item.pricing.amount;
        else {
            subtotal += item.pricing.amount;
            if (item.type === 'roman' && item.pricing.isOversized) oversizedCount += item.qty;
            if (item.pricing.extraRodCost) { rodCost += item.pricing.extraRodCost; subtotal -= item.pricing.extraRodCost; }
        }
    });

    const oversizedCost = oversizedCount > 0 ? 50 + (oversizedCount - 1) * 25 : 0;
    const grandTotal = subtotal + rodCost + shipping + oversizedCost + additionVal - deductionVal;
    const totals = [
        ['Products Subtotal', subtotal], ['Oversized Surcharge', oversizedCost],
        ['Hardware & Rods Cost', rodCost], ['Shipping & Delivery', shipping]
    ];
    if (additionVal > 0) totals.push([additionDesc, additionVal]);
    if (deductionVal > 0) totals.push([deductionDesc, -deductionVal]);
    totals.push(['GRAND TOTAL', grandTotal]);
    y += 32;
    const tw=520, tx=W-margin-tw;
    totals.forEach((r,i)=>{
        const isGrand=i===totals.length-1;
        ctx.fillStyle=isGrand?'#B55A30':'#fff'; ctx.fillRect(tx,y,tw,54);
        ctx.strokeStyle='#222'; ctx.strokeRect(tx,y,tw,54);
        ctx.beginPath();ctx.moveTo(tx+340,y);ctx.lineTo(tx+340,y+54);ctx.stroke();
        ctx.fillStyle=isGrand?'#fff':'#111'; ctx.font=`${isGrand?'700':'400'} 23px Arial, sans-serif`;ctx.textBaseline='middle';
        ctx.fillText(r[0],tx+14,y+27);ctx.textAlign='right';ctx.fillText((r[1]<0?'-$':'$')+Math.abs(r[1]).toFixed(2),tx+tw-14,y+27);ctx.textAlign='left';
        y+=54;
    });
    y+=38;

    ctx.strokeStyle='#333';ctx.beginPath();ctx.moveTo(margin,y);ctx.lineTo(W-margin,y);ctx.stroke();y+=20;
    ctx.font='700 22px Arial, sans-serif';ctx.fillStyle='#111';ctx.fillText('Terms and Conditions (BRAUN INTERNATIONAL LLC):',margin,y);y+=38;
    const terms=[
        '1. All custom-made window treatments are fabricated strictly to the dimensions provided above.',
        '2. Roman Shade prices are calculated on square footage with a minimum of 10 sqft per shade.',
        '3. Roman shades wider than 60" are subject to oversized shipping surcharges ($50 for the 1st, $25 for each subsequent piece).',
        '4. Draperies are calculated on finished width (min 20" finished width). Height surcharges: 104-132" +20%, 132-158" +50%, 158-198" +100%, 198-238" +150%, and over 238" up to 278" +200%.'
    ];
    ctx.font='21px Arial, sans-serif';
    terms.forEach(t=>{const ls=wrapCanvasText(ctx,t,W-margin*2);ls.forEach(line=>{ctx.fillText(line,margin,y);y+=28;});y+=8;});
    y+=45;
    ctx.strokeStyle='#111';ctx.beginPath();ctx.moveTo(margin,y);ctx.lineTo(margin+470,y);ctx.moveTo(W-margin-470,y);ctx.lineTo(W-margin,y);ctx.stroke();
    ctx.font='21px Arial, sans-serif';ctx.textAlign='center';ctx.fillText('Customer Signature / Date',margin+235,y+12);ctx.fillText('Factory Approval Signature',W-margin-235,y+12);ctx.textAlign='left';
    return canvas;
}

function downloadCanvasAsSinglePagePdf(jpegDataUrl, imgWidthPx, imgHeightPx, filename) {
    ...
    // Use points while preserving the long-image aspect ratio. Width is fixed to A4 width; height is extended.
    const pageW = 595.28;
    ...
    let offset = 0;
    const offsets = [0];
    const pushText = (s) => { const b = encoder.encode(s); chunks.push(b); offset += b.length; };
    const pushBytes = (b) => { chunks.push(b); offset += b.length; };
    const obj = (n, content) => { offsets[n] = offset; pushText(`${n} 0 obj\n${content}\nendobj\n`); };
...
// Fabrication bilingual labels (workshop-facing)
function getRomanStyleBilingual(style) {
    const map = {
        plain: 'Plain Fold 后凸款（前凸后凸 3cm）', ribbed: 'Ribbed Plain Fold 筋骨后凸款',
        flat: 'Flat B 平铺款（无明线）', hobbled: 'Hobbled 鱼鳞款（前凸后凸 3cm）',
        tdbu: 'TDBU 上下开合款', twin: 'Twin 双联鱼鳞折叠款',
        relaxed: 'Relaxed Waterfall 扇形水波款', tulip: 'Tulip Waterfall 郁金香水波款',
        balloon: 'Balloon Roman 气球波浪款'
    };
    return map[style] || String(style || '-');
}
function getLiningBilingual(lining) {
    const map = { blackout:'Blackout 全遮光内衬', privacy:'Light Filtering 半遮光/透光内衬',
        unlined:'Unlined 无内衬', none:'No Liner 无内衬', interlining:'Interlining 夹棉内衬' };
    return map[String(lining || '').toLowerCase()] || String(lining || '-');
}
function getMountBilingual(mount) {
    const key=String(mount||'').toUpperCase();
    return (key==='IM'||key==='IB') ? 'IB / IM 口内安装（成品宽自动减 1/4"≈6mm）' : 'OB / OM 口外安装（不减宽）';
}
function getLiftBilingual(lift) {
    const map={cordless:'Cordless Standard 标准无绳',ccl:'Continuous Cord Loop 循环拉绳',motorized:'Motorized AM25 电动'};
    return map[String(lift||'').toLowerCase()] || String(lift||'-');
}
function getInstructionsBilingual(text) {
    const raw=String(text||'').trim(); if(!raw) return '无 / None'; const lower=raw.toLowerCase();
    const known=[
        [/returns? one piece style|one piece return/,'整幅回折一片式 / Returns one piece style'],
        [/same fabric direction|as sample book direction/,'按样本册方向排版 / As sample book direction'],
        [/pattern match/,'对花加工 / Pattern match'],[/no visible stitch|no top stitch/,'正面无明线 / No visible top stitch'],
        [/left control/,'左侧控制 / Left control'],[/right control/,'右侧控制 / Right control']
    ];
    for(const [pattern,translated] of known){if(pattern.test(lower)) return translated;}
    return '加工备注：'+raw;
}
function refreshAllFabricationDetails(){currentOrderItems.forEach(item=>{if(item.type!=='manual') item.fabrication=calculateFabricationDetails(item);});}

// Populate Print Workshop Fabrication Template
function populatePrintFabTemplate() {
    refreshAllFabricationDetails();
        ...
        if (item.type === 'roman') {
            ...
            romanItemsHtml += `
                <tr>
                    ...
                    <td>宽: ${fab.w_cm} cm${fab.width_deduction_mm ? `<br><small>IB 已减 ${fab.width_deduction_mm}mm</small>` : ''}<br>高: ${fab.h_cm} cm</td>
                    <td>宽: ${fab.cut_w} cm<br><small>含左右各 5cm 包边</small><br>高: <strong>${fab.cut_h} cm</strong></td>
                    <td>${fab.folds}<br><small>档距 H=${fab.H}cm</small><br><small>${fab.foldGuideline}</small></td>
                    <td class="marks-text">${fab.marksStr}</td>
                    <td>${fab.cordStr}</td>
                    <td><span style="border:1px solid #000; padding:1px 4px;">${getMountBilingual(item.mount)}</span></td>
                    <td>${item.fabric_code} (组:${item.fabric_group})<br><small>供应商物料编码</small></td>
                    <td>${getRomanStyleBilingual(item.shade_style)}<br>${getLiftBilingual(item.lift_control)}</td>
                    <td>${getLiningBilingual(item.lining)}</td>
                    <td>${item.trim !== 'none' ? item.trim : '无'}</td>
                    <td style="white-space: pre-wrap;">${getInstructionsBilingual(item.special_instructions)}</td>
                </tr>
            `;
        } else if (item.type === 'drapery') {
            dCount++;
    ...
    template.innerHTML = `
        <div class="print-fab-sheet">
            <div class="print-fab-header">
                <div>
                    <h1>BRAUN INTERNATIONAL LLC 生产制造车间工艺单</h1>
...
function exportContractExcel() {
    ...
    let wsData = [
        ["BRAUN INTERNATIONAL LLC", "", "", "", "", "", "", "", "Proforma Invoice Form"],
        [],
        [],
        [],
        [],
        ...
        ["SIDEMARK:", "", "", sidemark],
        [],
        ["Roman Shades & Draperies Order (BRAUN INTERNATIONAL LLC)"],
...
function exportFabricationExcel() {
    ...
    let wsData = [
        ["BRAUN INTERNATIONAL LLC 车间定制工艺加工单", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""],
...
// BRAUN v3.0 complete fabrication output.
// Recalculates all production data, isolates the fabrication template,
// and prevents a blank first or trailing page.
function printFabricationComplete() {
    refreshAllFabricationDetails();
    ...
    if (!fabTemp || !fabTemp.innerHTML.trim()) {
        alert('加工图纸生成失败，请检查订单数据。');
        return;
    }

    document.body.classList.add('printing-fabrication');
...
window.addEventListener('afterprint', () => {
    document.body.classList.remove('printing-fabrication');
});
